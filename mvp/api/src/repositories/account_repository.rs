...
impl AccountRepository {
    pub async fn create(
        ...
        let row = sqlx::query(
            ...
            VALUES ($1, $2, $3, $4)
            RETURNING id, account_number, account_type, user_id, balance, currency, status, created_at, updated_at
            "#,
        )
        .bind(account_number)
    ...
    pub async fn find_by_id(pool: &PgPool, id: Uuid) -> Result<Account, AppError> {
        let row = sqlx::query(
            r#"
            SELECT id, account_number, account_type, user_id, balance, currency, status, created_at, updated_at
            FROM accounts
            WHERE id = $1
            "#,
        )
