...
public class UserController {
    ...
    public UserController(UserService userService, IdempotencyService idempotencyService, ObjectMapper objectMapper) {
    ...
    public ResponseEntity<?> createUser(@RequestBody CreateUserRequest body, HttpServletRequest request) {
        ...
        String idempotencyKey = (String) request.getAttribute(EnvironmentAndIdempotencyFilter.HEADER_IDEMPOTENCY_KEY);
        ...
        return idempotencyService
                .getCachedResponse(idempotencyKey, method, path, env, body.organizationId)
                .map(cached -> ResponseEntity.status(cached.status()).body(readJson(cached.jsonBody())))
                .orElseGet(() -> {
                    idempotencyService.recordInProgress(idempotencyKey, method, path, env, body.organizationId, body);
                    ...
                    idempotencyService.recordResponse(idempotencyKey, method, path, env, body.organizationId, HttpStatus.CREATED, resp);
