...
-- Add constraint to ensure customers must have an admin (except for admins themselves)
ALTER TABLE users
ADD CONSTRAINT chk_customer_has_admin
CHECK (
    (role = 'ADMIN' AND admin_user_id IS NULL) OR
    (role = 'CUSTOMER' AND admin_user_id IS NOT NULL) OR
    (role = 'SERVICE')
);

-- Add constraint to ensure admin_user_id references an ADMIN role user
ALTER TABLE users
ADD CONSTRAINT chk_admin_user_is_admin
CHECK (
    admin_user_id IS NULL OR
    admin_user_id IN (SELECT id FROM users WHERE role = 'ADMIN')
);

-- Create indexes for performance optimization
CREATE INDEX CONCURRENTLY idx_users_admin_customer ON users(admin_user_id)
WHERE admin_user_id IS NOT NULL;

CREATE INDEX CONCURRENTLY idx_users_org_role ON users(organization_id, role);

CREATE INDEX CONCURRENTLY idx_users_org_env_admin ON users(organization_id, environment, admin_user_id)
WHERE role = 'CUSTOMER';

-- Add organizational context column for better multi-tenancy
ALTER TABLE users
ADD COLUMN organizational_context JSONB DEFAULT '{}';

-- Create index on organizational context for efficient queries
CREATE INDEX CONCURRENTLY idx_users_org_context_gin ON users
...
CREATE POLICY admin_access ON users
    FOR ALL
    USING (
        -- Admins can access their own record and their customers
        CASE
...
CREATE MATERIALIZED VIEW user_organizational_analytics AS
SELECT
    organization_id,
    environment,
...
CREATE UNIQUE INDEX idx_user_org_analytics_pk ON user_organizational_analytics(organization_id, environment);
...
-- Note: These would be environment-specific in production
